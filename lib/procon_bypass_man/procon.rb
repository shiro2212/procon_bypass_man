...
class ProconBypassMan::Procon
  ...
  def apply!
    ...
    ProconBypassMan.logger.debug {"test:user_operation.binary.raw[13..48]:: #{user_operation.binary.raw[13..48]}"}
    ProconBypassMan.logger.debug {"test:user_operation.binary.raw[13]:: #{user_operation.binary.raw[13]}"}
    ...
    if ongoing_macro.finished? && enable_all_macro
      current_layer.macros.each do |macro_name, options|
        ...
        if(if_tilted_left_stick_value = options[:if_tilted_left_stick])
          ...
          degreeFrom = (if_tilted_left_stick_value.is_a?(Hash) && if_tilted_left_stick_value[:degreeFrom])
          degreeTo = (if_tilted_left_stick_value.is_a?(Hash) && if_tilted_left_stick_value[:degreeTo])
          isAngleRange = true
          isTilt = true
          if threshold
