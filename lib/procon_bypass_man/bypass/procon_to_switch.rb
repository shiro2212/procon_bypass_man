    ...
    ProconBypassMan::Procon::PerformanceMeasurement.measure do |measurement|
      ...
      next(run_callbacks(:work) {
        ...
        if !(external_input_data.nil?)
          # 配列長
          length = BlueGreenProcess::SharedVariable.instance.data["recent_accel_x"].size - 1
          # １F前の値
          latest = length - 1

          start = nil
          if external_input_data.raw_data.include?("_aim0_")
            start = 7
          elsif external_input_data.raw_data.include?("_aim2_")
            start = 5
          elsif external_input_data.raw_data.include?("_aim3_")
            start = 4
          elsif external_input_data.raw_data.include?("_aim4_")
            start = 3
          elsif external_input_data.raw_data.include?("_aim5_")
            start = 2
          ...
          if start
            gyro = []
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_x"][start].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_y"][start].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_z"][start].first]).pack("S*<")
            # accelX = 0
            # for i in start..latest
            ...
            gyro1 = 0
            for i in start..latest
              gyro1 += BlueGreenProcess::SharedVariable.instance.data["recent_gyro_1"][i].first
            end
            gyro <<  [gyro1 * -1].pack("S*<")
            gyro2 = 0
            for i in start..latest
              gyro2 += BlueGreenProcess::SharedVariable.instance.data["recent_gyro_2"][i].first
            end
            gyro <<  [gyro2 * -1].pack("S*<")
            gyro3 = 0
            for i in start..latest
          ...
          elsif external_input_data.raw_data.include?("_fix_")
            gyro = []
          ...
          elsif external_input_data.raw_data.include?("_hogex_")
            gyro = []
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_x"][latest].first + 200]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_y"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_z"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_gyro_1"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_gyro_2"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_gyro_3"][latest].first]).pack("S*<")
          elsif external_input_data.raw_data.include?("_hogey_")
            gyro = []
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_x"][latest].first]).pack("S*<")
            gyro <<  ([600]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_z"][latest].first]).pack("S*<")
            ...
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_gyro_2"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_gyro_3"][latest].first]).pack("S*<")
          elsif external_input_data.raw_data.include?("_hogez_")
            gyro = []
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_x"][latest].first]).pack("S*<")
            gyro <<  ([BlueGreenProcess::SharedVariable.instance.data["recent_accel_y"][latest].first]).pack("S*<")
            gyro <<  ([4400]).pack("S*<")
